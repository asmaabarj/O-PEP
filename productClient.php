...
<body>
    ...
    if (mysqli_num_rows($categoryResult) > 0) {
        while ($categoryRow = mysqli_fetch_assoc($categoryResult)) {
            $selected = ($selectedCategory == $categoryRow['idCategorie']) ? 'bg-green-500 text-white' : 'bg-white text-gray-800';
            echo '<button id="categoryButton_' . $categoryRow['idCategorie'] . '" onclick="filterPlants(' . $categoryRow['idCategorie'] . ')" class="px-4 py-2 m-2 rounded focus:outline-none ' . $selected . '">' . $categoryRow['nomCategorie'] . '</button>';
        }
    }
    ?>
</div>
            ...
            <div class="flex flex-wrap text-left lg:text-left justify-between mr-[50px]">
                <div class="w-full lg:w-6/12 px-4">
                    ...
                    <div class="mt-6 lg:mb-0 mb-6">
                        ...
                        <div class="mt-[10px]"><input type="text" id="email-address-icon" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block  pl-10 p-2.5  dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="name@flowbite.com"></div>
                    </div>
                </div>
                <div class="ml-[10vw]">
    ...
    function filterPlants(selectedCategory) {
        document.location.href = 'productClient.php?category=' + selectedCategory;
    }
